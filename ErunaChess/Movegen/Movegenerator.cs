...
namespace ErunaChess
{
	public static class MoveGenerator
	{
		public static void GenerateAllMoves(Board board, MoveList moveList)
		{
			...
			// generate black pawn moves (wrong D:)
			else
			{
				for (int i = 0; i < board.pieceCount[(int)Board.Pieces.blackPawn]; i++)
				{
					...
					if (square <= (int)Global.Square.H2)
					{
						...
						//captures
						if ((board.board[square - Global.boardWidth + 1] & Global.border) == enemy)
							AddMove.PromotionMove(board, Move.Write(square, board.board[square + Global.boardWidth + 1], board.board[square - Global.boardWidth + 1], 0, false, false, false), moveList);
					...
					else
					{
						...
						//captures
						if ((board.board[square - Global.boardWidth + 1] & Global.border) == enemy)
							AddMove.PawnCaptureMove(board, Move.Write(square, board.board[square + Global.boardWidth + 1], board.board[square - Global.boardWidth + 1], 0, false, false, false), moveList);
